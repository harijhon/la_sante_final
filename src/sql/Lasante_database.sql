...
--view detail produit (vue ngaze be an'le medicament)
-- vue FINAL vue ngaze be
create view product_data as (
    SELECT medicament.nom_medicament,
...
-- create view product_data as (
--     SELECT produit.id_medicament, FROM produit
--      JOIN medicament ON produit.id_medicament = medicament.id_medicament
--      JOIN forme ON medicament.id_forme = forme.id_forme
--      JOIN type_medicament ON medicament.id_type = type_medicament.id_type
--      JOIN marque ON medicament.id_marque = marque.id_marque
...
--   select nom_medicament,nom_forme,nom_type,nom_marque,produit.prix_unitaire,produit.quantite_medicament,
-- 	nom_admin,notice,pour_allaitement,pour_enceinte,lien_img,dose
-- 	from produit
-- 	join medicament_data on produit.id_medicament = medicament_data.id_medicament;
-- );
...
-- TANDREMOOOOOOOOOOOOOOOO
-- etat panier livraison
create table etat(
    id_etat int,
...
create table choixLivraison(
	...
	date_livraison DATE,
	id_typeLivraison int,
	FOREIGN KEY (id_pannierMere) REFERENCES pannierMere(id_pannierMere),
...
-- detail produit
create view product_data as (
    select produit.id_produit,prix_unitaire,produit.id_medicament,nom_medicament,lien_img,quantite_medicament from produit
    join medicament on produit.id_medicament = medicament.id_medicament
    join current_price on produit.id_produit = current_price.id_produit
);
